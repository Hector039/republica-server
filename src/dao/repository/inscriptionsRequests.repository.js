export default class InscriptionsRequestsRepository {
    ...
    addInscPayment = async ( iid, amount, payDate ) => {
      try {
        const sql = 'INSERT INTO `inscription_payments`(`id_inscription`, `amount`, `pay_date`) VALUES (?, ?, ?)';
        const [result, fields] = await this.database.execute(sql, [iid, amount, payDate]);
        return result;
      } catch (err) {
        throw err
      }
